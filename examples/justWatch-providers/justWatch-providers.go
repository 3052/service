...
func main() {
   ...
   if *jsonFile != "" {
      ...
      sort.Slice(sortedCountries, func(i, j int) bool {
         lenI, lenJ := len(sortedCountries[i].Providers), len(sortedCountries[j].Providers)
         if lenI != lenJ {
            return lenI > lenJ
         }
         return sortedCountries[i].Code < sortedCountries[j].Code
      })
      ...
      for _, countryInfo := range sortedCountries {
         ...
         foundSlugs := processCountry(countryInfo.Code, providerFilter)
